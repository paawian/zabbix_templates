# Apache by Zabbix agent template description
...
* [discoveries](#discoveries)
  ...
  * [Discovery Apache process discovery ](#discovery_apache_process_discovery)
...
| Service response time | no description | net.tcp.service.perf[http,"{$APACHE.STATUS.HOST}","{$APACHE.STATUS.PORT}"] | no type | no delay |
| Service ping | no description | net.tcp.service[http,"{$APACHE.STATUS.HOST}","{$APACHE.STATUS.PORT}"] | no type | no delay |
| Get processes summary | The aggregated data of summary metrics for all processes. | proc.get[{$APACHE.PROCESS.NAME.PARAMETER},,,summary] | no type | no delay |
| Get status | Getting data from a machine-readable version of the Apache status page.<br>For more information see Apache Module [mod_status](https://httpd.apache.org/docs/current/mod/mod_status.html). | web.page.get["{$APACHE.STATUS.SCHEME}://{$APACHE.STATUS.HOST}:{$APACHE.STATUS.PORT}/{$APACHE.STATUS.PATH}"] | no type | no delay |
