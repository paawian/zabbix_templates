# Windows by SNMP template description
...
| Windows: SNMP walk network interfaces | Discovering interfaces from IF-MIB. | net.if.walk | SNMP_AGENT | no delay |
...
| Windows: SNMP walk mounted filesystems | HOST-RESOURCES-MIB::hrStorage discovery. | vfs.fs.walk | SNMP_AGENT | no delay |
...
| {$NET.IF.IFDESCR.NOT_MATCHES} | Miniport\|Virtual\|Teredo\|Kernel\|Loopback\|Bluetooth\|HTTPS\|6to4\|QoS\|Layer\|isatap\|ISATAP |
...
| ------------- |------------- |------------- |------------- |------------- |------------- |
| Unavailable by ICMP ping | HIGH | Last three attempts returned timeout.  Please check device connectivity. | max(/Windows by SNMP/icmpping,#3)=0 | [{"tag": "scope", "value": "availability"}] | no url |
| High ICMP ping loss | WARNING | no description | min(/Windows by SNMP/icmppingloss,5m)>{$ICMP_LOSS_WARN} and min(/Windows by SNMP/icmppingloss,5m)<100 | [{"tag": "scope", "value": "availability"}, {"tag": "scope", "value": "performance"}] | no url |
| High ICMP ping response time | WARNING | Average ICMP response time is too high. | avg(/Windows by SNMP/icmppingsec,5m)>{$ICMP_RESPONSE_TIME_WARN} | [{"tag": "scope", "value": "availability"}, {"tag": "scope", "value": "performance"}] | no url |
| High CPU utilization | WARNING | CPU utilization is too high. The system might be slow to respond. | min(/Windows by SNMP/system.cpu.util,5m)>{$CPU.UTIL.CRIT} | [{"tag": "scope", "value": "performance"}] | no url |
| System name has changed | INFO | The name of the system has changed. Acknowledge to close the problem manually. | last(/Windows by SNMP/system.name,#1)<>last(/Windows by SNMP/system.name,#2) and length(last(/Windows by SNMP/system.name))>0 | [{"tag": "scope", "value": "notice"}, {"tag": "scope", "value": "security"}] | no url |
| No SNMP data collection | WARNING | SNMP is not available for polling. Please check device connectivity and SNMP settings. | max(/Windows by SNMP/zabbix[host,snmp,available],{$SNMP.TIMEOUT})=0 | [{"tag": "scope", "value": "availability"}] | no url |
| Host has been restarted | WARNING | Uptime is less than 10 minutes. | (last(/Windows by SNMP/system.hw.uptime[hrSystemUptime.0])>0 and last(/Windows by SNMP/system.hw.uptime[hrSystemUptime.0])<10m) or (last(/Windows by SNMP/system.hw.uptime[hrSystemUptime.0])=0 and last(/Windows by SNMP/system.net.uptime[sysUpTime.0])<10m) | [{"tag": "scope", "value": "notice"}] | no url |
...
| Mounted filesystem discovery | vfs.fs.discovery[snmp] | HOST-RESOURCES-MIB::hrStorage discovery with storage filter. | DEPENDENT | no lifetime | 0 |
| Memory discovery | vm.memory.discovery | HOST-RESOURCES-MIB::hrStorage discovery with memory filter | DEPENDENT | no lifetime | 0 |
...
| name | description | key | type |
| ------------- |------------- |------------- |------------- |
| FS [{#FSNAME}]: Space: Used, in % | The space utilization expressed in % for {#FSNAME}. | vfs.fs.pused[{#SNMPINDEX}] | DEPENDENT |
...
| FS [{#FSNAME}]: Space: Used | MIB: HOST-RESOURCES-MIB<br>The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits. | vfs.fs.used[hrStorageUsed.{#SNMPINDEX}] | DEPENDENT |
| FS [{#FSNAME}]: Get data | HOST-RESOURCES-MIB::hrStorage.<br>Intermediate data for subsequent processing. | vfs.fs.walk.data[hrStorage.{#SNMPINDEX}] | DEPENDENT |
...
| name | priority | description | expression | tags | url |
| ------------- |------------- |------------- |------------- |------------- |------------- |
| FS [{#FSNAME}]: Space is critically low | AVERAGE | The storage space usage exceeds the '{$VFS.FS.PUSED.MAX.CRIT:"{#FSNAME}"}%' limit. | min(/Windows by SNMP/vfs.fs.pused[{#SNMPINDEX}],5m)>{$VFS.FS.PUSED.MAX.CRIT:"{#FSNAME}"} | [{"tag": "scope", "value": "availability"}, {"tag": "scope", "value": "capacity"}] | no url |
| FS [{#FSNAME}]: Space is low | WARNING | The storage space usage exceeds the '{$VFS.FS.PUSED.MAX.WARN:"{#FSNAME}"}%' limit. | min(/Windows by SNMP/vfs.fs.pused[{#SNMPINDEX}],5m)>{$VFS.FS.PUSED.MAX.WARN:"{#FSNAME}"} | [{"tag": "scope", "value": "availability"}, {"tag": "scope", "value": "capacity"}] | no url |
...
| name | description | key | type |
| ------------- |------------- |------------- |------------- |
| {#MEMNAME}: Get data | MIB: HOST-RESOURCES-MIB<br>The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits. | vm.memory.data[{#SNMPINDEX}] | DEPENDENT |
| {#MEMNAME}: Used | MIB: HOST-RESOURCES-MIB<br>The amount of the storage represented by this entry that is allocated, in units of hrStorageAllocationUnits. | vm.memory.used[hrStorageUsed.{#SNMPINDEX}] | DEPENDENT |
| {#MEMNAME}: Utilization | Memory utilization in %. | vm.memory.util[{#SNMPINDEX}] | DEPENDENT |
| {#MEMNAME}: Total | MIB: HOST-RESOURCES-MIB<br>The size of the storage represented by this entry, in units of hrStorageAllocationUnits.<br>This object is writable to allow remote configuration of the size of the storage area in those cases where such an operation makes sense and is possible on the underlying system.<br>For example, the amount of main memory allocated to a buffer pool might be modified or the amount of disk space allocated to virtual memory might be modified. | vm.memory.walk.data.total[hrStorageSize.{#SNMPINDEX}] | DEPENDENT |
...
| name | priority | description | expression | tags | url |
| ------------- |------------- |------------- |------------- |------------- |------------- |
| {#MEMNAME}: High memory utilization | AVERAGE | The system is running out of free memory. | min(/Windows by SNMP/vm.memory.util[{#SNMPINDEX}],5m)>{$MEMORY.UTIL.MAX} | [{"tag": "scope", "value": "capacity"}, {"tag": "scope", "value": "performance"}] | no url |
